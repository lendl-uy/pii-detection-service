...
function saveNewDocument() {
    const documentContent = document.getElementById('document-viewer').value; // Use value for textarea
...
function verifyDocument(){
    console.log(data)
    const document_id = data.doc_id;
    axios.patch(`http://127.0.0.1:5000/documents/${document_id}`, data)
        .then(response => {
...
function renderTokens() {
    if (!data || !data.tokens) {
        console.log("Data or tokens are not available.");
        return; // Exit if data is not set up correctly
    }
...
function fetchDocuments() {
    ...
    .then(data => {
        // Assuming data is set here that affects the rendering
        ...
        renderTokens(); // Call renderTokens here if it depends on this data
    })
